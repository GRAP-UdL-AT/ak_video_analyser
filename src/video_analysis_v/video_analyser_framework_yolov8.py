...
from object_detection.generic_detector_yolov8_mask import ObjectDetectorYoloV8Mask
...
class VideoAnalyserFrameworkYoloV8:
    conf = None
    _a_matroska_file = None

    def __init__(self, config_param: VideoAnalyserConfigYoloV8, a_matroska_file):
        logging.debug('STARTING Frames Video Manager')
        if config_param is None:
            logging.debug('config_param is empty')
            self.conf = VideoAnalyserConfigYoloV8()
        else:
    ...
    def run_analysis_bbox(self, start_offset, number_of_frames=None):
        ...
        obj_detector = ObjectDetectorYoloV8Mask(self.conf.obj_det_options)
        ...
        try:
            ...
            while frames_checked < number_of_frames:
                ...
                if capture.color is not None and capture.transformed_depth is not None:
                    ...
                    depth_image_filtered, bit_mask_filtered = obj_depth_filter.depth_filter_2t(self.conf.filter_distance_min, self.conf.filter_distance_max)
                    ...
                    # Object detector
                    # -----------------------------
                    [new_boxes, new_scores, new_labels, np_array_mask, final_masks] = obj_detector.detection_in_frame(rgb_img_filtered)  # frame_to_show)

                    if new_boxes:
                        # count detected objects
                        # TODO: check coordinates filter
                        boxes_filtered, scores_filtered, labels_filtered, obj_counted, total_frame_objects, to_count = filter_obj.filter_list_by_coordinates(new_boxes, new_scores, new_labels)
                        ...
                        if boxes_filtered:
                            ...
                            data_feature_processor = DataFeatureProcessor(self.conf.data_features_options, capture.color, capture.transformed_depth)
                            results_by_frame_df = data_feature_processor.roi_selector_loop_bbox(boxes_filtered, labels_filtered)
                            # --------------------------------------------
    ...
    def run_analysis_mask(self, start_offset, number_of_frames=None):
        ...
        obj_detector_mask = ObjectDetectorYoloV8Mask(self.conf.obj_det_options)
        # the table to save results
        results_by_frame_df = pd.DataFrame([], columns=self.conf.data_features_options.header_frame_summary)
        frame_record_df = pd.DataFrame([], columns=self.conf.header_frame_summation)
        frame_summation_df = pd.DataFrame([], columns=self.conf.header_frame_summation)
        try:
            ...
            while frames_checked < number_of_frames:
                ...
                if capture.color is not None and capture.transformed_depth is not None:
                    ...
                    # Object detector
                    # -----------------------------
                    [new_boxes, new_scores, new_labels, np_array_mask, final_masks] = obj_detector_mask.detection_in_frame(rgb_img_filtered)  # frame_to_show)
    ...
    def export_analysis_mask(self, start_offset, number_of_frames=None):
        ...
        :param start_offset: number of seconds from the beginning
        :param number_of_frames: number of frames to extract. If this is None, we take length as value
        :return:
        """
        frames_checked = 0
        errors = "STRING_ERROR"
