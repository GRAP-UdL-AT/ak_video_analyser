...
class TestDatasetDrawCircles(unittest.TestCase):
    ...
    def test_draw_rotate_rectangle(self):
        BASE_FOLDER = os.path.abspath('')
        path_to_save_df = os.path.join(BASE_FOLDER, 'output_dataset', 'rectangles')
        # ---------------------
        ...
        for a_register in result_pair_list:
            ...
            # ----------------------------------------------------
            drawed_rectangle_image = screen_layout.draw_rotate_rectangle_frame(rgb_frame, ip_1, pv_labelled_list, pv_label_list)
            cv2.imshow('Circles', drawed_rectangle_image)
            cv2.waitKey()
            cv2.imwrite(os.path.join(path_to_save_df, img_filename + '_l' + '.png'), drawed_rectangle_image)
            # break
        # -----------------
