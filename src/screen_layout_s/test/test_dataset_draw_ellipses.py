...
class TestDatasetDrawEllipses(unittest.TestCase):
    ...
    def test_draw_ellipse_fitting_frame(self):
        BASE_FOLDER = os.path.abspath('')
        path_to_save_df = os.path.join(BASE_FOLDER, 'output_dataset', 'ellipses')
        # ---------------------
        ...
        for a_register in result_pair_list:
            ...
            drawed_ellipses_image = screen_layout.draw_ellipse_fitting_frame(rgb_frame, ip_1, pv_labelled_list, pv_label_list)
            cv2.imshow('image from disk', drawed_ellipses_image)
            cv2.waitKey()

            cv2.imwrite(os.path.join(path_to_save_df, img_filename + '_l' + '.png'), drawed_ellipses_image)
            # break
        # -----------------
